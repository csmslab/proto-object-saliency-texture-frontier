function [pyr1 msk1 pyr2 msk2] = vonMisesPyramidPlus(map, vmPrs)
...
for ori = 1:vmPrs.numOri
    for l = vmPrs.minLevel:vmPrs.maxLevel
        ...
        if ~(isempty(map(l)))
            ...
            maxValue=max(max(map(l).data));
            pyr1(l).orientation(ori).data = imfilter(map(l).data,msk_1,maxValue);
            ...
            pyr2(l).orientation(ori).data = imfilter(map(l).data,msk_2,maxValue);
